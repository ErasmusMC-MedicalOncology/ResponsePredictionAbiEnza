...
plots$TMB <- AbiEnza.Results$mutationalBurden %>% 
    ...
    dplyr::filter(hmfSampleId %in% trainingSamples) %>% 
...
plots$totalSV <- AbiEnza.Results$mutationalBurden %>% 
    ...
    ggpubr::geom_bracket(data = AbiEnza.Results$differencesWGS$MutationalBurden %>% dplyr::filter(variable == 'totalSV'), ggplot2::aes(xmin = group1, xmax = group2, label = p.adj.signif, fill = NULL, color = NULL, shape = NULL), y.position = 1, tip.length = .01) +
    ggplot2::scale_y_continuous(trans = scales::pseudo_log_trans(), expand = c(0,0), breaks = c(0, 25, 50, 100, 250, 500, 1000, 1500, 2000, 3000), limits = c(25, 3001)) +
    theme_Job
...
plots$SV.DEL <- AbiEnza.Results$mutationalBurden %>% 
    ...
    gghalves::geom_half_boxplot(side = 'l', alpha = .3, outlier.shape = NA, notch = F, show.legend = F) +
    gghalves::geom_half_point_panel(side = 'r', position = ggbeeswarm::position_quasirandom(width = .15), size = 1.5, shape = 21, color = 'black', lwd = .01) +
    ggplot2::scale_y_continuous(trans = scales::pseudo_log_trans(), expand = c(0,0), breaks = c(0, 5, 10, 25, 50, 100, 250, 500, 1000, 2000), limits = c(0, 2001)) +
    ggplot2::scale_fill_manual(values = colorPalette, breaks = unique(AbiEnza.Metadata$Responder), guide = guide_legend(title = NULL, title.position = 'top', title.hjust = 0.5, nrow = 1, keywidth = 0.5, keyheight = 0.5)) +
    ggplot2::stat_summary(fun = min, colour = 'black', geom = 'text', size = 3, show.legend = FALSE, angle = 0) +
    ...
    ggpubr::geom_bracket(data = AbiEnza.Results$differencesWGS$MutationalBurden %>% dplyr::filter(variable == 'SV.DEL'), ggplot2::aes(xmin = group1, xmax = group2, label = p.adj.signif, fill = NULL, color = NULL, shape = NULL), y.position = 1, tip.length = .01) +
    theme_Job
...
plots$SV.DUP <- AbiEnza.Results$mutationalBurden %>% 
    ...
    ggplot2::scale_shape_manual(values = c(21, 23)) +
    ggpubr::geom_bracket(data = AbiEnza.Results$differencesWGS$MutationalBurden %>% dplyr::filter(variable == 'SV.DUP'), ggplot2::aes(xmin = group1, xmax = group2, label = p.adj.signif, fill = NULL, color = NULL, shape = NULL), y.position = 1, tip.length = .01) +
    theme_Job
...
data.Arms <- data.Arms[rownames(data.Arms) %in% (AbiEnza.Metadata %>% dplyr::filter(hmfSampleId %in% trainingSamples) %>% dplyr::pull(sampleId)),]
