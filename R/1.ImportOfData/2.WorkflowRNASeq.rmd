...
```{R, eval = F}
files <- data.frame(R1 = list.files('/mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/fastq/', pattern = 'R1.*fastq.gz', recursive = T, full.names = T), stringsAsFactors = F) %>% dplyr::mutate(R2 = gsub('_R1_', '_R2_', R1), outR1 = file.path('/mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/trimmedReads/', gsub('.fastq.gz', '_fastpTrimmed.fastq.gz', basename(R1))), outR2 = file.path('/mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/trimmedReads/', gsub('.fastq.gz', '_fastpTrimmed.fastq.gz', basename(R2))))
...
for BAM in /mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/BAM/*bam
do echo "sambamba markdup -t 10 ${BAM} ${BAM%.out.bam}_markDup.bam";
done

# Flagstats. (sambamba 0.7.0)
for BAM in /mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/BAM/*sorted*_markDup.bam
...
```{bash, eval =F}
/mnt/data/ccbc_environment/software/general/subread-2.0.0-Linux-x86_64/featureCounts -T 50 -t exon -g gene_id --primary -p -s 2 -a /mnt/data/ccbc_environment/general/annotation/hg19/GENCODE/noChrPrefix_VEP_gencode.v35lift37.annotation.gtf -o /mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/counts/DR71_RNA.counts /mnt/data2/hartwig/DR71/Oct2020/dataHMF/RNASeq/BAM/*sorted*markDup.bam
```
